...
Prioritas thread Java berada dalam rentang antara `MIN_PRIORITY` (konstanta 1) dan `MAX_PRIORITY` (konstanta 10). Secara default, setiap thread diberi prioritas `NORM_PRIORITY` (konstanta 5). Semakin tinggi nilai prioritas, semakin penting thread tersebut dianggap oleh sistem operasi. Sistem operasi kemudian akan berusaha memberikan CPU lebih banyak kepada thread dengan prioritas lebih tinggi daripada yang memiliki prioritas lebih rendah. 
